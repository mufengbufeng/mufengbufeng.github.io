...
Fluid.plugins = {
  ...
  codeWidget() {
    ...
    var enableFold = CONFIG.code_fold && CONFIG.code_fold.enable;
    if (!enableLang && !enableCopy && !enableFold) {
      return;
    }
    ...
    // 代码折叠功能
    function addCodeFold($element) {
      if (!enableFold) return;
      
      // 确保元素已渲染
      setTimeout(function() {
        var elementHeight = $element.outerHeight();
        var maxHeight = CONFIG.code_fold.max_height || 400;
        
        if (elementHeight > maxHeight) {
          // 包装代码块
          var $wrapper = $element.wrap('<div class="code-fold collapsed"></div>').parent();
          
          // 添加折叠按钮
          var $foldBtn = jQuery('<button class="code-fold-btn">▼ 展开</button>');
          $wrapper.append($foldBtn);
    ...
    jQuery('.markdown-body pre, .markdown-body figure.highlight').each(function() {
      var $element = jQuery(this);
      var $pre = $element.is('pre') ? $element : $element.find('pre').first();
      ...
      // 添加代码折叠功能
      addCodeFold($element.is('figure.highlight') ? $element : $pre);
    });
  }
};
